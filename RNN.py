...
with g.as_default():
    ...
    for i in range(num_unrollings):
        ...
        output_data_feed=output_data_feed.append(output_after)
        
    tensor_real_returns=[input_data[i][0] for i in range(num_unrollings)]
    #tensor_real_returns=tf.concat(tensor_real_returns,tensor_real_returns_unrollings)
    
    observed_returns=np.multiply(output_data_feed,tensor_real_returns)+delta*np.abs(np.array(output_data_feed[1:])-np.array(output_data_feed[0:-1]))
